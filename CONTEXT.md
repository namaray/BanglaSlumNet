# BanglaSlumNet — AI context bundle

This document is meant to be pasted or uploaded into an AI model as **full project context**. It summarizes goals, architecture, data layout, dependencies, and **complete source code** for all Python modules in the repository (as of generation). Binary assets (GeoTIFFs, shapefiles, zips) are **not** embedded here; only their roles and naming patterns are described.

---

## 1. Project title and scientific goal

**Name:** BanglaSlumNet  

**Full research framing:** Physics-guided atmospheric disentanglement with cross-modal socioeconomic fusion for decadal informal settlement mapping in Bangladesh.

**Problem addressed (South Asian remote sensing):**

1. **Atmospheric haze** — brick-kiln smog and seasonal haze; Stage 1 uses a SAS-Net–style disentanglement (structure vs appearance) plus a PatchGAN-style discriminator.
2. **Optical ambiguity** — dense formal housing vs slums; Stage 2 fuses satellite structure features with socioeconomic rasters (night lights, population, building footprints) via multi-head cross-attention.
3. **Temporal inconsistency** — year-to-year flicker in predictions; Stage 3 applies a 2D ConvLSTM over time and a SAR-guided temporal smoothness loss so changes are allowed when radar indicates real structural change.

**Target geography / era:** Bangladesh informal settlements; documentation references **2015–2025** and Dhaka-area experiments (Mirpur, Korail, Old Dhaka).

---

## 2. Repository layout (code vs data)

### 2.1 Python source files (canonical list)

| File | Role |
|------|------|
| `sasnet.py` | Structure encoder (EfficientNet-B2–based, 4-channel input), appearance encoder, AdaIN. |
| `stage1_gan.py` | PatchGAN discriminator, `SASNetLoss` (reconstruction, adversarial, structure consistency). |
| `train_stage1.py` | Paired hazy/clear dataset, `Stage1Generator`, Stage 1 GAN training loop. |
| `stage2_fusion.py` | Per-channel SE encoders, `CrossAttentionFusion` (MHA + residual). |
| `decoder.py` | U-Net-style upsampling decoder → single-channel sigmoid slum probability map. |
| `train_stage2.py` | Multi-modal dataset, `Stage2Loss` (soft IoU + weighted BCE), Stage 2 training. |
| `stage3_temporal.py` | `ConvLSTMCell`, `TemporalModule`, SAR-masked `TemporalLoss`. |
| `train_stage3.py` | 5D temporal dataset, end-to-end Stage 3 loop (frozen Stage 1–2 feature extraction + temporal + decoder). |
| `slum_dataloader.py` | Early/simple `BanglaSlumDataset` for single-stack GeoTIFFs. |
| `download_s2.py` | GEE: single Sentinel-2 tile export (4 bands). |
| `download_pairs.py` | GEE: hazy + clear pairs for three named locations → `paired_dataset/`. |
| `download_socioeconomic.py` | GEE: NTL, WorldPop, Google Open Buildings rasters aligned to same tiles. |
| `download_temporal.py` | GEE: multi-year S2 + SAR “no change” masks → `temporal_dataset/`. |
| `download_dhaka_dataset.py` | GEE: 50 random tiles over Dhaka administrative boundary, multi-modal exports → `dhaka_dataset/`. |
| `view_tile.py` | Quick RGB visualization of a local GeoTIFF. |
| `inference_export.py` | CPU inference stub, morphological closing, GeoTIFF + shapefile export. |

### 2.2 Typical on-disk data folders (not committed as code)

- **`paired_dataset/`** — `{mirpur,korail,old_dhaka}_{hazy,clear,ntl,pop,gob}.tif` (from pair + socioeconomic scripts).
- **`temporal_dataset/`** — `{loc}_s2_{year}.tif`, `{loc}_sar_{y1}_{y2}.tif`.
- **`dhaka_dataset/`** — `dhaka_NN_{hazy,clear,ntl,pop,gob}.tif` for NN in 01..50.
- **`final_outputs/`** — example inference outputs (`korail_slum_prediction.tif`, `korail_slum_polygons.shp`, etc.).

### 2.3 Tensor / file conventions

- Sentinel-2 bands in code: **B4, B3, B2, B8** (R, G, B, NIR) → **4 × 512 × 512** tensors after crop.
- Normalization: reflectance-like values divided by **3000**, clipped to **[0, 1]** for optical stacks.
- GeoTIFFs are read with **rasterio**; first **512×512** crop is used consistently.

---

## 3. Pipeline architecture (high level)

```text
GeoTIFF (4ch, 512²)
    → StructureEncoder → s_m [B,256,64,64]     (Stage 1 also uses AppearanceEncoder + AdaIN + image decoder for de-hazing GAN)
    → CrossAttentionFusion(s_m, SE rasters) → F [B,256,64,64]
    → SegmentationDecoder(F) → mask logits/probs [B,1,512,512]

Temporal extension:
    Per-year F_t from frozen struct_enc + fusion
    → stack → TemporalModule (ConvLSTM) → H_t [B,T,128,64,64]
    → SegmentationDecoder(in_channels=128) per timestep
    → Stage2Loss per year + TemporalLoss(pred sequence, SAR mask)
```

**Important implementation notes for readers:**

- `CrossAttentionFusion` is authored for **5** SE channels in the docstring; `train_stage2.py` / `train_stage3.py` / `inference_export.py` use **`num_se_channels=3`** (NTL, Pop, GOB) to match downloaded data.
- `train_stage2.py` and `train_stage3.py` bypass the decoder’s final sigmoid and use **`decoder.final_conv[0]`** only so **`BCEWithLogitsLoss`** can be applied; `inference_export.py` uses the **full** decoder including sigmoid.
- `train_stage3.py` passes **`torch.sigmoid(pred_seq)`** into `TemporalLoss` while SAR mask is binary; comments mention “10-year” validation but the loop uses **3** years (2021–2023).

---

## 4. External services and configuration

- **Google Earth Engine:** Scripts call `ee.Initialize(project=project_id)` with placeholder `project_id = 'banglaslumnet-research'`. You must set this to your GEE project.
- **Python packages:** `requirements.txt` exists (pip freeze style). It lists **torch**, **numpy**, **opencv-python**, **scikit-image**, etc. It may **not** list every import used by geospatial scripts (e.g. **rasterio**, **geemap**, **earthengine-api**, **geopandas**, **shapely**, **matplotlib**). Install those explicitly if missing.
- **Hardware:** README mentions CUDA 12.x and RTX-class GPU for training; code falls back to CPU when CUDA is unavailable.

---

## 5. README milestones (condensed)

- **Mission 1:** GEE export, dataloader, SAS-Net + fusion + decoder skeleton, forward pass.
- **Mission 2:** Paired hazy/clear data, PatchGAN, SASNet losses, `train_stage1.py` loop.
- **Mission 3:** Socioeconomic rasters, Stage 2 IoU + BCE-style loss, `train_stage2.py`.
- **Mission 4:** Temporal S2 + S1 masks, ConvLSTM, temporal loss, `train_stage3.py`.

**README roadmap (still open in doc):** tightened handoff deps, inference at scale, national run, etc. **Note:** `requirements.txt` and `inference_export.py` exist in the tree; README checklist may be partially outdated.

---

## 6. `requirements.txt` (full file)
...
## 7. Complete Python source

### 7.1 `slum_dataloader.py`
...
### 7.12 `download_socioeconomic.py`
...
### 7.14 `download_dhaka_dataset.py`
...
## 8. How to use this bundle

- **For coding agents:** Point them at the live files in the repo for edits; use this file when the model **cannot** access the workspace.
- **For long-context models:** You may need to split sections **7.x** across multiple messages if the model has a smaller context window.
- **Refresh:** After substantive code changes, regenerate or append a short “delta” section so the bundle stays truthful.

---

*End of BanglaSlumNet CONTEXT.md*
